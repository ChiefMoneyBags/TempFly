...
public class PageAskyblock extends Page {
	...
	public PageAskyblock(GuiSession session) {
		super(session);
		AskyblockHook hook = (AskyblockHook) TempFly.getHooks().getHook(Hook.ASKYBLOCK);
	...
	public void runPage(int slot, InventoryClickEvent e) {
		...
		if (island == null) {
			// Im a static abuser
			U.m(p, AskyblockHook.requireIsland);
			session.endSession();
			return;
		}
		IslandSettings settings = ((AskyblockHook)TempFly.getHooks().getHook(Hook.ASKYBLOCK)).getIslandSettings(island);
		switch(slot) {
		case 11:
		case 20:
