...
public class LegacyDataFile {
	 ...
	 * @param plugin
	 */
	protected static void formatYamlData(DataBridge bridge, FileConfiguration data) {
		...
		if (version < 2.0) {
			...
			if (!backupLegacyData(bridge, data,"update_2_backup_")) {
				Bukkit.getPluginManager().disablePlugin(bridge.getTempFly());
				return;
			}
			...
			if (!backupLegacyData(bridge, data,"update_3_backup_")) {
				Bukkit.getPluginManager().disablePlugin(bridge.getTempFly());
				return;
			}
			...
			if (!backupLegacyData(bridge, data, "update_4_backup_")) {
				Bukkit.getPluginManager().disablePlugin(bridge.getTempFly());
				return;
			}
	 ...
	 * @return
	 */
	protected static boolean backupLegacyData(DataBridge bridge, FileConfiguration data, String file) {
		Console.info("Creating a backup of your data file...");
		File f = new File(bridge.getTempFly().getDataFolder(), file + String.valueOf(new Random().nextInt(99999)) + ".yml");
		try {
			data.save(f);
		} catch (Exception e) {
